...
return (
  <div className={depth > 0 ? 'ml-3 sm:ml-4' : ''}>
    <button
      ...
      className={`w-full flex items-center justify-between px-3 py-2.5 text-left transition-colors ${
        activeTab === item.id 
          ? `bg-${theme.accent}-500/20 text-${theme.accent}-400 font-bold` 
        ...
        {item.submenus.map(sub => (
          <button
            ...
            className={`w-full flex items-center gap-1.5 px-3 py-2 ml-5 sm:ml-7 text-left text-xs sm:text-sm transition-colors ${
              activeTab === sub.id 
                ? `text-${theme.accent}-400 font-bold` 
  ...
  return (
<>
  <header className="fixed top-0 inset-x-0 z-50 h-14 sm:h-16 border-b border-white/5 bg-black/50 backdrop-blur-xl flex items-center justify-between px-3 sm:px-4 lg:px-6">
    <div className="flex items-center gap-2 sm:gap-6">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 font-bold text-white tracking-tight cursor-pointer" onClick={() => setActiveTab(menuItems[0].id)}>
        <div className={`w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-${theme.accent}-500/20 flex items-center justify-center text-${theme.accent}-500`}>
    ...
    <div className="flex items-center gap-1.5 sm:gap-3">
      ...
      <button onClick={onLogout} className="flex items-center gap-1.5 sm:gap-2 group">
        <div className="text-right hidden lg:block">
          ...
          <div className={`text-[9px] sm:text-[10px] text-${theme.accent}-400 uppercase whitespace-nowrap`}>
      ...
      <div className="fixed top-0 left-0 h-full w-64 sm:w-72 bg-[#0A0A0A] border-r border-white/10 z-50 md:hidden overflow-y-auto animate-fade-in-up">
        <div className="flex items-center justify-between p-3 border-b border-white/10">
          <div className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-lg bg-${theme.accent}-500/20 flex items-center justify-center text-${theme.accent}-500`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-2.5 border-t border-white/10 bg-[#0A0A0A]">
          <div className="flex items-center gap-2 mb-2">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className={`text-[10px] text-${theme.accent}-400 whitespace-nowrap overflow-hidden text-ellipsis`}>
                {role === 'athlete' ? 'Diamond II' : language === 'ko' ? '헤드코치' : 'Head Coach'}
              </div>
            </div>
          </div>
