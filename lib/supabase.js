...
function enrichPlayerWithMatchTier(profile, stats) {
  ...
  const mp = computeMatchPoints(wins, draws);
  return {
    ...profile,
    wins,
    draws,
...
function enrichPublicPlayerRow(row) {
  ...
  const mp = computeMatchPoints(row.wins, row.draws);
  return {
    ...row,
    match_points: mp,
    tier: getTierLabelFromMatchPoints(mp),
...
export const getUserProfile = async (userId) => {
  try {
    // 1) 본인 세션: sql/15_get_my_profile_rpc.sql 의 SECURITY DEFINER RPC 우선 (RLS 이슈 회피)
    const { data: authUserRes, error: authUserErr } = await supabase.auth.getUser();
    const authId = authUserRes?.user?.id;
    if (!authUserErr && authId && authId === userId) {
      const { data: rpc, error: rpcErr } = await supabase.rpc('get_my_profile');
      if (!rpcErr && rpc && rpc.ok === true && rpc.user) {
        const normalized = {
          ...rpc.user,
          user_private_profiles: rpc.private ? [rpc.private] : [],
        };
        const flat = flattenProfile(normalized);
        const data = await attachStatisticsAndTier(userId, flat);
        return { data, error: null };
      }
      if (rpcErr) {
        console.warn('[getUserProfile] get_my_profile RPC 실패, 테이블 조회로 재시도:', rpcErr.message);
      }
    }
...
function mapInvestError(code) {
  const m = {
    ...
    already_unlocked:
      '같은 노드는 최대 5번까지 강화할 수 있습니다. 두 번째부터 막히면 Supabase에서 invest_skill_node 함수가 최신인지 확인하세요. (저장소 sql/10 또는 sql/11_fix_invest_already_unlocked.sql 실행)',
    ...
    max_investments_reached: '이 노드는 최대 5번까지 찍을 수 있습니다.',
  };
