...
export const getUserMatches = async (userId, limit = null) => {
  try {
    ...
    const normalizedData = (data || []).map((match) => ({
      ...match,
      opponent_name: match?.opponent?.nickname || match?.opponent?.name || null,
    }));
...
export const addMatch = async (matchData) => {
  try {
    ...
    return {
      data: {
        ...data,
        opponent_name: data?.opponent?.nickname || data?.opponent?.name || null,
      },
      error: null,
    };
  } catch (error) {
...
export const getMatchLeaderboard = async (limit = 200) => {
  try {
    ...
    (matches || []).forEach((match) => {
      if (!match?.user_id || !match?.played_at) return;
      if (!latestMatchByUser[match.user_id]) {
        latestMatchByUser[match.user_id] = match.played_at;
      }
    });
...
export const investSkillNodeRpc = async (nodeId) => {
  try {
    const { data, error } = await supabase.rpc('invest_skill_node', { target_node_id: nodeId });
...
export const getGymPromotionRequestsDetailed = async (status = null) => {
  try {
    ...
    const [{ data: users }, { data: forks }] = await Promise.all([
      supabase.from('users').select('id, name, nickname, email').in('id', userIds),
      supabase
        .from('skill_tree_nodes')
...
function mapInvestError(code) {
  const m = {
    ...
    already_unlocked: '이미 포인트를 쓴 노드입니다.',
