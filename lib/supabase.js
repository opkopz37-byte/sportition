...
export const submitMatchResult = async ({
  ...
  try {
    ...
    const methodLabel = finishMethod === 'rsc' ? 'RSC' : finishMethod === 'forced' ? 'forced_stop' : 'decision';
    ...
    await Promise.all([
      updateStatistics(blueUserId, blueResult, methodLabel === 'RSC' && blueResult === 'win'),
      updateStatistics(redUserId, redResult, methodLabel === 'RSC' && redResult === 'win'),
    ]);
