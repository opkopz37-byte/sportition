...
// (sql/46_attendance_rpc_kst.sql 적용 필요)
//   - 클라이언트 toISOString() = UTC 날짜 버그 제거
//   - 멱등 — 같은 날 여러 번 눌러도 안전
//   - attendance INSERT + statistics + users.skill_points 한 트랜잭션
export const checkAttendance = async (_userId = null, _location = null) => {
  try {
    ...
    return {
      data,
      error: null,
      message: data?.message || (alreadyChecked ? '이미 출석' : '출석 체크 완료!'),
      skillPointsAdded: alreadyChecked ? 0 : 1,
      totalSkillPoints: typeof data?.skill_points === 'number' ? data.skill_points : null,
...
/** 노드별 EXP 진행 (0~5, 재심사 거절 시 최대 10) + 거절 횟수.
 *  promotion_fail_count 컬럼이 없는 환경 (sql/49 미적용) 도 graceful 하게 동작. */
export const getUserSkillNodeProgress = async (userId) => {
  try {
    // 1차: 새 컬럼 포함해 조회
    const first = await supabase
      .from('user_skill_node_progress')
      .select('node_id, exp_level, promotion_fail_count, updated_at')
      .eq('user_id', userId);
    if (!first.error) return { data: first.data, error: null };

    // 2차 폴백: 컬럼 없는 옛 스키마 — exp_level 만
    console.warn('[getUserSkillNodeProgress] promotion_fail_count 없음 — 폴백 SELECT', first.error?.message);
    const fallback = await supabase
      .from('user_skill_node_progress')
      .select('node_id, exp_level, updated_at')
      .eq('user_id', userId);
    if (fallback.error) throw fallback.error;
    // promotion_fail_count 기본 0 으로 채움
    const data = (fallback.data || []).map((r) => ({ ...r, promotion_fail_count: 0 }));
    return { data, error: null };
  } catch (error) {
