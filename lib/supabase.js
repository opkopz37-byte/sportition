...
// [deprecated] 출석 시 활성 스킬 EXP 자동 적립으로 대체됨. RPC 도 sql/71 에서 no-op.
// 정의는 유지 (구버전 import 호환). 새 코드는 호출하지 말 것.
export const claimDailySkillPoint = async () => {
  if (typeof console !== 'undefined') {
    console.warn('[claimDailySkillPoint] deprecated — 출석 시 활성 스킬 EXP 자동 적립으로 대체됨');
  }
  return { data: null, error: { message: 'deprecated' } };
};
...
// [deprecated] 회원의 직접 EXP 적립은 폐지 — 출석 시 활성 스킬 자동 적립으로 대체됨.
// RPC 도 sql/71 에서 no-op. 정의는 유지 (구버전 import 호환).
export const addSkillExpRpc = async () => {
  if (typeof console !== 'undefined') {
    console.warn('[addSkillExpRpc] deprecated — 회원 직접 EXP 적립 폐지');
  }
  return { data: null, error: { message: 'deprecated' } };
};
...
// [deprecated] 회원의 직접 투자/해금은 폐지 — 관장이 해금/스킵 처리.
// RPC 도 sql/71 에서 no-op. 정의는 유지 (구버전 import 호환).
export const investSkillNodeRpc = async () => {
  if (typeof console !== 'undefined') {
    console.warn('[investSkillNodeRpc] deprecated — 회원 직접 투자 폐지');
  }
  return { data: null, error: { message: 'deprecated' } };
};

// [deprecated] 회원의 직접 해금은 폐지 — 관장이 해금 처리.
export const unlockSkillNodeRpc = async () => {
  if (typeof console !== 'undefined') {
    console.warn('[unlockSkillNodeRpc] deprecated — 회원 직접 해금 폐지');
  }
  return { data: null, error: { message: 'deprecated' } };
};
