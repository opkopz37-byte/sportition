...
export const getUserProfile = async (userId) => {
  ...
  fetch('http://127.0.0.1:7851/ingest/5b41cc5d-64b3-441c-ba49-522a7c7a8930', {
    ...
    body: JSON.stringify({
      sessionId: '546a3d',
      location: 'supabase.js:getUserProfile',
      message: 'entry',
      data: { userId, hypothesisId: 'D' },
      timestamp: Date.now(),
    }),
  }).catch(() => {});
  // #endregion
  try {
    ...
    fetch('http://127.0.0.1:7851/ingest/5b41cc5d-64b3-441c-ba49-522a7c7a8930', {
      ...
      body: JSON.stringify({
        sessionId: '546a3d',
        location: 'supabase.js:getUserProfile',
        message: 'after_getUser',
        data: {
          ms: Date.now() - t0,
          authMatch: authUserRes?.user?.id === userId,
          authErr: !!authUserErr,
          hypothesisId: 'D',
        },
        timestamp: Date.now(),
      ...
      if (!rpcErr && rpc && rpc.ok === true && rpc.user) {
        ...
        fetch('http://127.0.0.1:7851/ingest/5b41cc5d-64b3-441c-ba49-522a7c7a8930', {
          ...
          body: JSON.stringify({
            sessionId: '546a3d',
            location: 'supabase.js:getUserProfile',
            message: 'done_rpc_path',
            data: { ms: Date.now() - t0, hypothesisId: 'C' },
            timestamp: Date.now(),
          }),
        }).catch(() => {});
        // #endregion
    ...
    fetch('http://127.0.0.1:7851/ingest/5b41cc5d-64b3-441c-ba49-522a7c7a8930', {
      ...
      body: JSON.stringify({
        ...
        location: 'supabase.js:getUserProfile',
        message: 'catch',
        data: { ms: Date.now() - t0, errMsg: error?.message, hypothesisId: 'C' },
        timestamp: Date.now(),
      }),
    }).catch(() => {});
    // #endregion
