...
function enrichPlayerWithMatchTier(profile, stats) {
  ...
  const mp = Number(profile.tier_points) || 0;
  return {
    ...profile,
    wins,
    draws,
...
export const checkAttendance = async (_userId = null, _location = null) => {
  try {
    ...
    if (data && typeof data === 'object' && data.ok === false) {
      // 서버가 message 를 보내면 그걸 우선 (예: 'no_active_skill' 에 대한 친절 안내)
      const msg = data.message || data.error || '출석 체크 실패';
      return { data, error: { message: msg, code: data.error }, message: msg };
    }
...
// claimDailySkillPoint 폐지됨 — 출석 시 활성 스킬 EXP 자동 적립으로 대체. RPC 는 sql/71 에서 no-op.
...
export const openAttendanceModal = async () => {
  try {
    ...
    // 1차에서 ok=false 가 명시되면 (활성 스킬 없음 등) 친절 메시지 + code 함께 반환
    if (!rpcRes.error && rpcRes.data && rpcRes.data.ok === false) {
      const d = rpcRes.data;
      const msg = d.message || d.error || '출석 처리 실패';
      return { data: d, error: { message: msg, code: d.error } };
    }
    const isMissing = rpcRes.error && /could not find|does not exist|404/i.test(String(rpcRes.error.message || ''));
    if (rpcRes.error && !isMissing) {
...
export const submitMatchResult = async ({
  ...
  try {
    ...
    // NC(무효) — 양쪽 모두 'nc' 로 저장, 승률·티어 점수 집계에서 제외됨
    const isNC = winnerCorner === 'nc' || finishMethod === 'nc';
    const normalizedWinner = winnerCorner === 'draw' ? 'draw' : winnerCorner;
    const blueResult = isNC ? 'nc' : normalizedWinner === 'draw' ? 'draw' : normalizedWinner === 'blue' ? 'win' : 'loss';
    const redResult = isNC ? 'nc' : normalizedWinner === 'draw' ? 'draw' : normalizedWinner === 'red' ? 'win' : 'loss';
    ...
    // matches 테이블(실제 소스)에서 전적을 다시 세어 저장 — 증분(+1) 방식이 아니므로
    // 동일 결과가 중복 호출돼도 값이 어긋나지 않음 (재호출해도 항상 같은 값으로 수렴)
    // 티어 점수(tier_points)는 단순 집계가 아니라 경기 순서대로 재생하며 0 밑을 막는 잔고 방식으로 계산
    const updateStatistics = async (userId) => {
      const { data: userMatches, error: fetchError } = await supabase
        .from('matches')
        .select('result, method, played_at')
        .eq('user_id', userId)
        .order('played_at', { ascending: true });

      if (fetchError) throw fetchError;

      const rows = userMatches || [];
      // NC(무효) 는 경기 수·승률 집계에서 제외 (목록에는 남음)
      const counted = rows.filter((m) => m.result !== 'nc');
      const nextStats = {
        user_id: userId,
        total_matches: counted.length,
        wins: counted.filter((m) => m.result === 'win').length,
        losses: counted.filter((m) => m.result === 'loss').length,
        draws: counted.filter((m) => m.result === 'draw').length,
        ko_wins: counted.filter((m) => m.result === 'win' && ['RSC', 'TKO', 'KO'].includes(m.method)).length,
      };
      const { total: tierPoints } = computeRunningPoints(rows);

      const [{ error: upsertError }, { error: userUpdateError }] = await Promise.all([
        supabase.from('statistics').upsert([nextStats], { onConflict: 'user_id' }),
        supabase.from('users').update({ tier_points: tierPoints }).eq('id', userId),
      ]);
    ...
    await Promise.all([
      updateStatistics(blueUserId),
      updateStatistics(redUserId),
    ]);
...
const SKILL_TREE_NODES_TTL_MS = 12 * 60 * 60 * 1000; // 12시간 (스킬 트리는 거의 안 바뀜)
const SKILL_TREE_NODES_STORAGE_KEY = 'skill_tree_nodes_cache_v1';
export const invalidateSkillTreeNodesCache = () => {
  _skillTreeNodesCache = null;
  try {
    if (typeof sessionStorage !== 'undefined') {
      sessionStorage.removeItem(SKILL_TREE_NODES_STORAGE_KEY);
    }
  } catch { /* ignore */ }
};
...
export const getSkillTreeNodes = async () => {
  try {
    ...
    // 2) sessionStorage 캐시 (페이지 reload 시에도 유효)
    try {
      if (typeof sessionStorage !== 'undefined') {
        const raw = sessionStorage.getItem(SKILL_TREE_NODES_STORAGE_KEY);
        if (raw) {
          const parsed = JSON.parse(raw);
          if (parsed?.ts && Date.now() - parsed.ts < SKILL_TREE_NODES_TTL_MS && Array.isArray(parsed.data)) {
            _skillTreeNodesCache = { data: parsed.data, ts: parsed.ts };
    ...
    if (error) throw error;
    const ts = Date.now();
    _skillTreeNodesCache = { data, ts };
    try {
      if (typeof sessionStorage !== 'undefined') {
        sessionStorage.setItem(SKILL_TREE_NODES_STORAGE_KEY, JSON.stringify({ data, ts }));
      }
...
// addSkillExpRpc 폐지됨 — 회원 직접 EXP 적립 불가. RPC 는 sql/71 에서 no-op.
...
// investSkillNodeRpc / unlockSkillNodeRpc 폐지됨 — 관장이 해금/스킵 처리. RPC 는 sql/71 에서 no-op.
...
export const playerUnlockSkillNodeRpc = async (nodeId) => {
  try {
    ...
    const { data, error } = await supabase.rpc('player_unlock_skill_node', { p_node_id: targetId });
    if (error) throw error;
    if (data && typeof data === 'object' && data.ok === false) {
      return { data: null, error: { message: mapPlayerUnlockError(data.error, data.message), code: data.error } };
    }
    return { data, error: null };
  } catch (error) {
    console.error('[playerUnlockSkillNodeRpc]', error);
    return { data: null, error };
  }
};

function mapPlayerUnlockError(code, fallback) {
  const m = {
    not_authenticated: '로그인이 필요합니다.',
    forbidden: '회원만 직접 스킬을 해금할 수 있습니다.',
    invalid_node: '존재하지 않는 스킬입니다.',
    already_unlocked: '이미 해금된 스킬입니다.',
    active_skill_in_progress: '진행 중인 스킬을 먼저 마스터해 주세요.',
    promotion_required: '마스터한 스킬의 승단 심사가 통과되기 전에는 새 스킬을 진행할 수 없습니다.',
    parent_not_mastered: '부모 스킬을 먼저 마스터해 주세요.',
  };
  return m[code] || fallback || '스킬을 해금할 수 없습니다.';
}
...
// mapInvestError 는 investSkillNodeRpc 폐지(deprecated)와 함께 제거됨
...
// 경기 이벤트 저장(덮어쓰기) — 해당 매치의 기존 이벤트 삭제 후 현재 목록으로 재삽입.
// 결과 저장 직후 1회 + 결과 화면에서 수정 시 재호출. 실패해도 경기 기록에 영향 없음.
export const replaceMatchEvents = async ({ blueMatchId, redMatchId, events = [] }) => {
  if (!blueMatchId || !redMatchId) return;
  try {
    const { error: delError } = await supabase
      .from('match_events')
      .delete()
      .eq('match_id_blue', blueMatchId)
      .eq('match_id_red', redMatchId);
    if (delError) console.error('[replaceMatchEvents] 기존 이벤트 삭제 실패:', delError);

    if (events.length > 0) {
      const { error } = await supabase.from('match_events').insert(
        events.map(e => ({
          match_id_blue: blueMatchId,
          match_id_red:  redMatchId,
          round_number:  e.round,
          elapsed_sec:   e.elapsedSec ?? null,
          event_type:    e.eventType,
          actor_corner:  e.actorCorner,
          recv_corner:   e.recvCorner,
        }))
      );
      if (error) console.error('[replaceMatchEvents] 이벤트 저장 실패:', error);
    }
  } catch (error) {
    console.error('[replaceMatchEvents] 이벤트 저장 실패:', error);
  }
};

// 경기별 이벤트 건수 일괄 조회 — 전적 목록에서 "타임라인 있음" 표시용 (쿼리 1회)
export const fetchMatchEventCounts = async (matchIds = []) => {
  ...
  try {
    ...
    const { data } = await supabase
      .from('match_events')
      .select('match_id_blue,match_id_red')
      .or(`match_id_blue.in.(${list}),match_id_red.in.(${list})`);
    const idSet = new Set(ids);
    const counts = {};
    (data || []).forEach((row) => {
      [row.match_id_blue, row.match_id_red].forEach((id) => {
...
// 경기 이벤트 타임라인 조회 (match_events 테이블)
export const fetchMatchEvents = async (matchId) => {
  try {
    const { data } = await supabase
      .from('match_events')
      .select('*')
      .or(`match_id_blue.eq.${matchId},match_id_red.eq.${matchId}`)
