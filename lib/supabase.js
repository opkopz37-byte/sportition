  ...
  try {
    ...
    const updateStatistics = async (userId) => {
      ...
      const nextStats = {
        ...
        wins: rows.filter((m) => m.result === 'win').length,
        losses: rows.filter((m) => m.result === 'loss').length,
        draws: rows.filter((m) => m.result === 'draw').length,
        ko_wins: rows.filter((m) => m.result === 'win' && ['RSC', 'TKO', 'KO'].includes(m.method)).length,
      };
