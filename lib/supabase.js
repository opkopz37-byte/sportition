...
export const uploadUserAvatarBlob = async (userId, jpegBlob) => {
  try {
    ...
    if (!publicUrl) throw new Error('공개 URL을 만들 수 없습니다.');
...
async function fetchPlayerDisplayByIds(userIds) {
  ...
  // avatar_url 은 별도로 users 테이블에서 조회 (public_player_profiles 뷰에 없음)
  ...
  try {
    const { data: avatarRows } = await supabase
      ...
      .in('id', ids);
    avatarMap = new Map((avatarRows || []).map((r) => [r.id, r.avatar_url || null]));
  } catch {
    // avatar_url 컬럼이 없거나 RLS 거부된 경우 fallback (아바타 없음)
  }

  return new Map((data || []).map((p) => [p.id, { ...p, avatar_url: avatarMap.get(p.id) || null }]));
}
...
/** 스킬 페이지·맵에 필요한 컬럼만 (payload 축소) */
const SKILL_TREE_NODES_SELECT =
  'id, node_number, name, name_en, zone, position_x, position_y, node_type, parent_nodes, map_lane, map_subtitle, is_milestone, display_title, source_name, description, training_intent, flow_summary, is_fork, point_cost';
...
export const getUserSkillNodeProgress = async (userId) => {
  try {
    const { data, error } = await supabase
      ...
      .select(
        'node_id, investment_count, promotion_status, chosen_branch_node_number, promotion_fail_count, updated_at'
      )
      .eq('user_id', userId);
