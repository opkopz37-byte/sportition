...
export function computeMatchRecords(matchesDesc) {
  ...
  chron.forEach((m, idx) => {
    if (m.result === 'nc') return; // 무효 경기 — 승률·연승 등 모든 집계 미반영
  ...
  // 현재 연승 (가장 최근 매치부터 거꾸로 — 패/무 만나면 중단)
  const desc = [...chron].reverse();
  let currentStreak = 0;
  for (const m of desc) {
    if (m.result === 'nc') continue; // 무효 경기는 연승을 끊지 않음
    if (m.result === 'win') currentStreak += 1;
    else break;
  }
...
export function normalizeRawMatch(m) {
  ...
  const dateLabel = playedAt && !Number.isNaN(playedAt.getTime())
    ...
    : '-';
  const result = m.result === 'win' || m.result === 'loss' || m.result === 'draw' || m.result === 'nc' ? m.result : 'draw';
  const oppNickname = m.opponent?.nickname || null;
  const oppRealName = m.opponent?.name || null;
  // 메인 표시명 = 닉네임 우선
  const displayOpponent = m.opponent_name || oppNickname || oppRealName || '상대 미상';
  // 보조 표시명 = 닉네임과 다른 실명만 (괄호 안)
