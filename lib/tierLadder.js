...
/**
 * 전적 기준 누적 랭크 점수 — 잔고 방식.
 * 0에서 시작해 경기 순서대로(played_at 오름차순) 한 경기씩 반영하며,
 * 매 경기마다 0 밑으로는 내려가지 않게 막는다 (승/무는 항상 0 이상이라 클램프 영향 없음, 패만 해당).
 * 단순 총합 후 한 번만 0 바닥을 씌우면 "이미 0인 상태에서 진 패배"가 이후 승리에서 몰래 깎이는 오류가 생기므로
 * 반드시 경기 단위로 클램프해야 함.
 *
 * @param matchesAsc { result: 'win'|'draw'|'loss' } 배열, played_at 오름차순 정렬된 상태로 전달
 * @returns { total, deltas } total = 최종 잔고, deltas = 경기별로 실제 적용된 변동폭(Map, key는 배열 인덱스)
 */
export function computeRunningPoints(matchesAsc) {
  let total = 0;
  const deltas = [];
  (matchesAsc || []).forEach((m) => {
    // nc(무효) 등 승/무/패 외 결과는 점수 무변동
    const raw = m.result === 'win' ? MATCH_POINTS_WIN
      : m.result === 'draw' ? MATCH_POINTS_DRAW
      : m.result === 'loss' ? MATCH_POINTS_LOSS
      : 0;
    const next = Math.max(0, total + raw);
    deltas.push(next - total);
    total = next;
  });
  return { total, deltas };
}
