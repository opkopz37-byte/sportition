...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // #region agent log
    fetch('http://127.0.0.1:7370/ingest/43671701-629e-4938-aa99-95f0adb6738f',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'cbb7b1'},body:JSON.stringify({sessionId:'cbb7b1',location:'AuthContext.js:24',message:'Supabase 설정 여부 체크',data:{isConfigured:isSupabaseConfigured()},hypothesisId:'A',runId:'init',timestamp:Date.now()})}).catch(()=>{});
    // #endregion
  ...
  const checkUser = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        loadUserProfile(currentUser.id, currentUser).catch(err => {
