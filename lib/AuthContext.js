...
import { supabase, getCurrentUser, getUserProfile, isSupabaseConfigured, debugLogDe1eee } from './supabase';
...
export const AuthProvider = ({ children }) => {
      ...
      async (event, session) => {
        ...
        if (event === 'INITIAL_SESSION') {
          ...
          try {
            ...
            debugLogDe1eee('AuthContext.js:INITIAL_SESSION:finally', 'INITIAL_SESSION_finally', {
              hypothesisId: 'A',
              runId: 'post-fix',
  ...
  const checkUser = async () => {
    ...
    try {
      ...
      debugLogDe1eee('AuthContext.js:checkUser:finally', 'checkUser_finally', { hypothesisId: 'A', runId: 'post-fix' });
      setLoading(false);
    }
  };
  ...
  const loadUserProfile = async (userId, authUser = null) => {
    ...
    const run = (async () => {
      ...
      try {
        ...
        debugLogDe1eee('AuthContext.js:loadUserProfile', 'after_getUserProfile', {
          hypothesisId: 'B',
          runId: 'post-fix',
          userId,
          hasData: Boolean(data),
          errMsg: error ? String(error.message || error).slice(0, 160) : null,
        });
        ...
        if (newProfile && !createError) {
          ...
          debugLogDe1eee('AuthContext.js:loadUserProfile:upsert', 'client_upsert_failed', {
            hypothesisId: 'D',
            runId: 'post-fix',
            userId,
            createErr: createError ? String(createError.message || createError).slice(0, 160) : null,
          });
          setProfile(null);
        }
      } catch (error) {
        ...
        debugLogDe1eee('AuthContext.js:loadUserProfile:catch', 'loadUserProfile_exception', {
          hypothesisId: 'B',
          runId: 'post-fix',
