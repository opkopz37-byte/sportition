...
export const AuthProvider = ({ children }) => {
  ...
  /** profileLoading: loadUserProfile 진행 중 여부 (세션 확인과 별개) */
  const [profileLoading, setProfileLoading] = useState(false);
  /** isOnline: 브라우저 네트워크 연결 상태 */
  ...
  // profileLoading 5초 타임아웃: 응답 없으면 강제 종료 후 에러 메시지 설정
  useEffect(() => {
    if (!profileLoading) return;
    const t = setTimeout(() => {
      console.warn('[AuthContext] 프로필 로딩 5초 초과 — 강제 종료');
      setProfileLoading(false);
      setProfileLoadError((prev) =>
        prev ?? '서버 응답이 너무 오래 걸립니다. 네트워크 상태를 확인하고 다시 시도해 주세요.'
      );
    }, 5000);
    return () => clearTimeout(t);
  }, [profileLoading]);
  ...
  // 네트워크 복구 감지: 프로필 미로딩 상태이면 자동 재시도
  useEffect(() => {
    if (!isOnline) return;
    if (!user || profile || profileLoading) return;
    authDevLog('[AuthContext] 네트워크 복구 감지 — 프로필 자동 재시도');
    loadUserProfile(user.id, user);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- isOnline 변경 시만 실행 (loadUserProfile은 안정적 참조)
  }, [isOnline]);
