...
/**
 * POST /api/gym-members/[memberId]/skill-unlock
 * Body: { node_id: number }
 * 응답 (성공): { ok: true, node_id, action: 'unlock' }
 * 응답 (실패): { ok: false, error, message? }
 *
 * 권한: gym (자기 회원) 또는 admin
 * 비즈니스 로직: RPC public.gym_unlock_skill_node 에서 처리 (활성 스킬 검사, 승단 신청 검사 등)
 */
export async function POST(request, ctx) {
  const ip = getClientIp(request);
  const rl = checkRateLimit(`${ip}:gym-skill-unlock`, { limit: 30, windowMs: 60_000 });
  if (!rl.ok) {
  ...
  // anon client + caller token 으로 RPC 호출 (RPC 내부 auth.uid() = 관장 id)
  ...
  const { data, error } = await supabase.rpc('gym_unlock_skill_node', {
